            ...
            success:function (res) {
                ...
                switch (res.status) {
                    ...
                    case 0:
                        ...
                        let lastTime = res.data.resource.updateTime;
                        let createTime = res.data.resource.createTime;
                        ...
                        $("#author").text(author.realName == null ? author.account : author.realName);
                        $(".lastTime").text(lastTime == null ? createTime : lastTime);
                        $("#downloadTimes").text(res.data.resource.downloadNum+"次");
...
function download(url) {
    let token = $.cookie(USER_DIGEST_COOKIE);
    if(token == null){
        systemAlert("red","请登录后下载");
    }else{
        downloadFile = false;   // 开启页面保护，防止用户关闭页面
        let fileName = url.substring(url.lastIndexOf(".")-13,url.length);
        ...
        $.ajax({
            url:HOST + COMMON.PUB_PREFIX + API.RESOURCE + "/update_download_num",
            type:"put",
            headers:{
