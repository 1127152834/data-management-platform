...
function renderResult(data) {
    var total = data.resources.length + data.articles.length + data.notices.length + data.links.length;
    if (total !=0){
        ...
        if(data.links.length !=0){
            $("#resultContainer").append("<div class=\"card-container\" >\n" +
                "                    <div class=\"card shadow\">\n" +
                "                        <h5 class=\"card-header\">外链 </h5>\n" +
                "                        <ul id=\"linkList\"></ul>\n" +
                "                    </div>\n" +
                "                </div>")
            for (var i in data.links) {
                var type = data.links[i].linkType == "1" ? "工具" : data.links[i].linkType == "2" ? "文章" : "其他";
                $("#linkList").append("<li class=\"result-list\">\n" +
                    "                    <h4 class=\"result-title\"><a target='_blank' href='"+data.links[i].linkAddress+"'>"+data.links[i].linkName+"</a></h4>\n" +
                    "                    <span class=\"result-type-container\">\n" +
                    "                                    <span class=\"badge badge-pill badge-info\">"+type+"</span>\n" +
                    "                                </span>\n" +
                    "                </li>")
