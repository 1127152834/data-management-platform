...
import com.zhang.datamanagementplatform.entity.POJO.VO.ResourceArticleVO;
import com.zhang.datamanagementplatform.entity.POJO.VO.ResourceListVO;
import com.zhang.datamanagementplatform.entity.POJO.VO.ResourceVO;
...
import com.zhang.datamanagementplatform.service.ResourceService;
...
public class ResourceServiceImp implements ResourceService {
        ...
        try{
            ...
            if (resource == null){
                ...
                if (resourceDao.deleteByUserIdAndId(id,userId)){
                    ...
                    String picPath = resource.getImgUrl();
                    if (!fileUtil.delFile(localPath)){
                        logger.warn("删除数据库记录成功，但删除资源文件失败，file_path={}",localPath);
                    }
                    if (!fileUtil.delFile(picPath)){
                        logger.warn("删除资源封面图片失败，file_path={}",localPath);
                    }
                }
            }
    ...
    @Override
    public ResultVO updateDownloadNum(String id) {
        try{
            resourceDao.updateDownloadNum(id);
            return resultUtil.success();
        }catch (Exception e){
            e.printStackTrace();
            return resultUtil.error(ResultEnum.UNKNOWN_ERROR.getStatus(),e.getMessage());
